...
class Ordini(Base):
    ...
    id = Column(Integer, Identity(always=True, start=1, cycle=False), primary_key=True)
    ...
    while total_op < 11:
        ...
        try:
            with Session() as session:
                ...
                if action == "UPDATE":
                    update_order = session.query(Ordini).get(
                        random.randint(1, next_num - 1)
                    )
                    update_order.amount = random.uniform(1, 100)
                    print(
                        f"Update dell'ordine {update_order.order_number} per ammontare {update_order.amount}"
                    )
                    session.commit()
